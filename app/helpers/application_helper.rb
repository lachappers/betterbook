module ApplicationHelper
    ...
    def notification_find(notifiable, type)
      return User.find(notifiable.notifiable_id) if type == 'friendRequest'
      return Post.find(notifiable.notifiable_id) if type == 'comment'
      return Post.find(notifiable.notifiable_id) if type == 'like-post'
      return unless type == 'like-comment'
      comment = Comment.find(notifiable.notifiable_id)
    ...
    def flash_classes(flash_type)
      ...
      {
        notifiable: "bg-indigo-600 #{flash_base}",
    ...
    def profile_image(user, options={})
      ...
      if user.profile_image.attached?
        ...
        image_tag "https://doodleipsum.com/700/avatar-5?bg=3D27F6i=f339578a64040310d3eb5bd82b550627", class: classes
      end
    end
